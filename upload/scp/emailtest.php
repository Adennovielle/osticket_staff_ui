...
if ($_POST) {
    ...
    if (!$errors && $email) {
        if ($email->send(
            ...
            null,
            array('reply-tag' => false)
        )) {
            $msg = Format::htmlchars(sprintf(
                __('Test email sent successfully to <%s>'),
                $_POST['email']
            ));
...
$ost->addExtraHeader(
    '<meta name="tip-namespace" content="emails.diagnostic" />',
...
<form action="emailtest.php" method="post" class="save">
    ...
    <table class="outgoing-email-table" border="0" cellspacing="0" cellpadding="2">
        <thead>
            <tr>
                <th colspan="2">
        ...
        <tbody>
            <tr>
                ...
                <td>
                    <select name="email_id" style="width: 100%;">
                        <option value="0">&mdash; <?php echo __('Select FROM Email'); ?> &mdash;</option>
                        <?php

                        $emails = Email::objects()->values_flat(
                            'email_id',
                            'email',
                            'name',
                            'smtp__active'
            ...
            <tr>
                ...
                <td>
                    <input type="text" style="width: 100%;" size="60" name="subj" value="<?php echo $info['subj']; ?>">
            ...
            <tr>
                <td colspan=2>
                    <div style="padding-top:0.5em;padding-bottom:0.5em">
                        <em><strong><?php echo __('Message'); ?></strong>: <?php echo __('email message to send.'); ?></em> sd;<span class="error"><?php echo $errors['message']; ?></span>
                    </div>
...
<script>
    if (window.innerWidth <= 900) {
        const outgoingEmailTable = document.querySelector(".outgoing-email-table");
        outgoingEmailTable.style.width = "100%";
    } else {
        const outgoingEmailTable = document.querySelector(".outgoing-email-table");
        outgoingEmailTable.style.display = "block";
    }
</script>
<?php
