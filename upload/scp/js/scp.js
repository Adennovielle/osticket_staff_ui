...
function checkbox_checker(formObj, min, max) {
  ...
  if (max > 0 && checked > max) {
    ...
    msg =
      msg + __("Please remove {0} selection(s).").replace("{0}", checked - max);
...
var scp_prep = function () {
  ...
  $("table.list input:checkbox").bind("click, change", function () {
    $(this).parents("tr:first").toggleClass("highlight", this.checked);
  });
  ...
  $(
    "#actions:submit, #actions :submit.button:not(.no-confirm), #actions .confirm",
  ).bind("click", function (e) {
    var formObj,
      name = this.name || $(this).data("name");
    if ($(this).data("formId")) formObj = $("#" + $(this).data("formId"));
    else formObj = $(this).closest("form");
    if ($(".dialog#confirm-action p#" + name + "-confirm").length === 0) {
      ...
      $(".dialog#confirm-action").delegate(
        "input.confirm",
        "click.confirm",
        function (e) {
          e.preventDefault();
  ...
  var warnOnLeave = function (el) {
    ...
    if (!fObj.data("changed")) {
      ...
      $("input[type=submit], button[type=submit]", fObj).addClass(
        "save pending",
      );
  ...
  $("form.save").on(
    ...
    function () {
      ...
      if (fObj.data("changed")) {
        $("input[type=submit], button[type=submit]", fObj).removeClass(
          "save pending",
        );
  ...
  //Canned attachments.
  $(".canned_attachments, .faq_attachments").delegate(
    "input:checkbox",
    "click",
    function (e) {
      var elem = $(this);
      if (
        !$(this).is(":checked") &&
        confirm(__("Are you sure you want to remove this attachment?")) == true
      ) {
  ...
  $("form select#cannedResp").on("select2:opening", function (e) {
    ...
    if (redactor) redactor.api("selection.save");
  });
  ...
  $("form select#cannedResp").change(function () {
    ...
    if (tid) $url = "ajax.php/tickets/" + tid + "/canned-resp/" + cid + ".json";
  ...
  $(".email.typeahead").typeahead({
    ...
    onselect: function (obj) {
      ...
      if (obj.name) $(".auto.name", fObj).val(obj.name);
    },
    property: "email",
  });
  ...
  $(".sortable-rows").sortable({
    ...
    stop: function (e, ui) {
      ...
      warnOnLeave(ui.item);
      $("input[name^=" + attr + "]", ui.item.parent("tbody")).each(
        function (i, el) {
          $(el).val(i + 1 + offset);
        },
      );
  ...
  $("div.sticky.bar:not(.stop)").each(function () {
    var $that = $(this),
      ...
      offset = $that.offset(),
      top =
        offset.top - parseFloat($that.css("marginTop").replace(/auto/, 100)),
    ...
    $(".content", $that).append(
      $(
        '<a class="only sticky scroll-up" href="#" data-stop=' +
          (placeholder.offset().top - 75) +
          ' ><i class="icon-chevron-up icon-large"></i></a>',
      ),
    );
  ...
  $(
    "div.tab_content[id] div.error:not(:empty), div.tab_content[id] font.error:not(:empty)",
  ).each(function () {
  ...
  $(function () {
    ...
    $("#customQ_nav").overflowmenu({
      ...
      change: function (e, ui) {
        var handle = ui.container.find(
          ".jb-overflowmenu-menu-secondary-handle",
        );
...
$(document).on(
  "focus",
  "form.spellcheck textarea, form.spellcheck input[type=text]",
  function () {
    var $this = $(this);
    if ($this.attr("lang") !== undefined) return;
    var lang = $(this).closest("[lang]").attr("lang");
    if (lang) $(this).attr({ spellcheck: "true", lang: lang });
  },
);

$(document).on("click", ".thread-entry-group a", function () {
  var inner = $(this).parent().find(".thread-entry-group-inner");
  if (inner.is(":visible")) inner.slideUp();
  else inner.slideDown();
  return false;
});
...
$.dialog = function (url, codes, cb, options) {
  ...
  if (codes && !$.isArray(codes)) codes = [codes];

  var $popup = $(".dialog#popup");

  $popup.attr("class", function (pos, classes) {
  ...
  $("div#popup-loading", $popup)
    .show()
    .find("h1")
    .css({
      "margin-top": function () {
        return $popup.height() / 3 - $(this).height() / 3;
      },
    });
  $popup.resize().show();
  $("div.body", $popup).load(url, options.data, function () {
    ...
    $(document).on("submit.dialog", ".dialog#popup form", function (e) {
      ...
      if (submit_button) {
        data +=
          "&" +
          escape(submit_button.attr("name")) +
          "=" +
      ...
      $.ajax({
        ...
        success: function (resp, status, xhr) {
          if (
            ...
            $("div.tab_content[id] div.error:not(:empty)", $popup).each(
              function () {
                var div = $(this).closest(".tab_content");
                $('a[href^="#' + div.attr("id") + '"]')
                  .parent()
  ...
  $.dialog($(this).data("dialog"), 201, function (xhr, json) {
    ...
    if (link.attr("href").length > 1) {
      ...
      if (typeof json === "object")
        link.attr(
          "href",
          link.attr("href").replace(/\{([^}]+)\}/, function ($0, $1) {
            return json[$1];
          }),
        );
...
$.sysAlert = function (title, msg, cb) {
  ...
  if ($dialog.length) {
    ...
    $dialog.resize().show();
    if (cb) $dialog.find("input.ok.close").click(cb);
  } else {
    alert(msg);
  }
};
...
$.confirm = function (message, title, options) {
  ...
  var body = $("div.body", $popup)
    ...
    .append($("<hr/>"))
    .append($('<p class="confirm-action"></p>').text(message))
    .append(
      $("<div></div>").append($("<b>").text(__("Please confirm to continue."))),
    );

  if (Object.keys(options).length) body.append("<hr>");
  $.each(options, function (k, v) {
    body.append(
      $("<div>")
        .html("&nbsp;" + v)
        .prepend($('<input type="checkbox">').attr("name", k)),
    );
  });
  ...
  body
    ...
    .append(
      $('<p class="full-width"></p>')
        .append(
          $('<span class="buttons pull-left"></span>').append(
        ...
        .append(
          $('<span class="buttons pull-right"></span>').append(
            $('<input type="button"/>')
              .attr("value", __("OK"))
              .click(function () {
                hide();
  ...
  if (ids.length) {
    var submit = function (data) {
      ...
      if (data)
        $.each(data, function () {
          form.append(
            $('<input type="hidden">').attr("name", this.name).val(this.value),
          );
        });
    ...
    $.sysAlert(__("Oops"), __("You need to select at least one item"));
  }
};
...
$.userLookup = function (url, cb) {
  $.dialog(
    ...
    201,
    function (xhr, user) {
      if ($.type(user) == "string") user = $.parseJSON(user);
...
$.orgLookup = function (url, cb) {
  $.dialog(
    ...
    201,
    function (xhr, org) {
      if ($.type(org) == "string") org = $.parseJSON(user);
      if (cb) cb(org);
    },
    {
      onshow: function () {
...
+(function ($) {
  var MessageBar = function () {
    ...
    this.show = function (title, message, options) {
      ...
      if (options.avatar)
        bar.prepend(
          $('<div class="avatar pull-left" title="Oscar"></div>').addClass(
            options.avatar,
          ),
        );
      ...
      if (options.onok || options.dismissible) {
        bar.prepend(
          $('<div><div class="valign-helper"></div></div>')
          ...
          function () {
            if (!hovering && this.visible && bar.css("top") != "0") {
              bar
                .stop()
                .animate(
                  { "margin-top": -parseInt(bar.css("top"), 10) },
                  400,
                  "easeOutBounce",
                );
              hovering = true;
    ...
    this.scroll = function (event) {
      ...
      if (!this.visible || this.options.position != "top") return;
    ...
    this.dismiss = function (event) {
      if (this.options.onok) {
        this.bar
          .find("button")
          .replaceWith($('<i class="icon-spinner icon-spin icon-large"></i>'));
        if (this.options.onok(event) === false) return;
      }
      this.hide();
    };

    this.hide = function () {
      if (!this.bar || !this.visible) return;
      ...
      bar.hide("slide", { direction: dir }, 400, function () {
        bar.remove();
      });
      this.visible = false;
    };
...
$(document).on("click.tab", "ul.tabs > li > a", function (e) {
  ...
  if (!$container.length) $container = $ul.parent();

  var $tab = $($this.attr("href"), $container);
  if (!$tab.length && $(this).data("url").length > 1) {
    var url = $this.data("url");
    if (url.charAt(0) == "#") url = "ajax.php/" + url.substr(1);
    $tab = $("<div>")
      .addClass("tab_content")
      .attr("id", $this.attr("href").substr(1))
      .hide();
    $container.append(
...
$(document).on(
  "click",
  "a.collaborator, a.collaborators:not(.noclick)",
  function (e) {
    e.preventDefault();
    var url = "ajax.php/" + $(this).attr("href").substr(1);
    $.dialog(
...
if ($.support.pjax) {
  $(document).on("click", "a", function (event) {
    ...
    if (
      ...
      $.pjax.click(event, {
        container: $this.data("pjaxContainer") || "#pjax-container",
        timeout: 30000,
      });
  });
}
...
$(document).on("change", "select[data-quick-add]", function () {
  ...
  if (
    !type ||
    (selected.data("quickAdd") === undefined && selected.val() !== ":new:")
  )
    return;
  $.dialog("ajax.php/admin/quick-add/" + type, 201, function (xhr, data) {
    data = JSON.parse(data);
    if (data && data.id && data.name) {
      var id = data.id;
      if (selected.data("idPrefix")) id = selected.data("idPrefix") + id;
      $("<option>").attr("value", id).text(data.name).insertBefore(selected);
      $select.val(id);
    }
  });
});
...
$(document).on("click.note", ".quicknote .action.edit-note", function (e) {
  ...
  if (note.closest(".dialog, .tip_box").length) T.addClass("no-bar small");
  body.replaceWith(T);
...
$(document).on("click.note", ".quicknote .delete", function () {
  if (!window.confirm(__("Confirm Deletion"))) return;
  var that = $(this),
...
$(document).on("click", "#new-note", function () {
  ...
  button.click(function () {
    $.post(
      "ajax.php/" + note.data("url"),
      {
        note: T.redactor("source.getCode"),
        no_options: note.hasClass("no-options"),
      },
      function (response) {
        T.redactor("stop");
        T.replaceWith(note);
...
function __(s) {
  if ($.oststrings && $.oststrings[s]) return $.oststrings[s];
  return s;
}
    ...
    return (
      (day_diff == 0 &&
        ((diff < 60 && __("just now")) ||
          ...
          (diff < 3600 &&
            __("%d minutes ago").replace("%d", Math.floor(diff / 60))) ||
          (diff < 7200 && __("about an hour ago")) ||
          (diff < 86400 &&
            __("%d hours ago").replace("%d", Math.floor(diff / 3600))))) ||
...
jQuery(function ($) {
  ...
  $.fn.show = function () {
    ...
    if (typeof arg === "number") options.duration = arg;
    else options.effect = arg;
  ...
  $.dialog(
    ...
    function (xhr) {
      ...
      if (obj.id && obj.value) {
        ...
        else $("#field_" + obj.id).removeClass("faded");
