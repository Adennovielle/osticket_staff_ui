...
<form method="post" action="dashboard.php">
  ...
  <ul class="clean tabs">
    ...
    foreach ($groups as $g => $desc) { ?>
      <li class="<?php echo $first ? 'active' : ''; ?>"><a href="#<?php echo Format::slugify($g); ?>"><?php echo Format::htmlchars($desc); ?></a></li>
    <?php
      $first = false;
    } ?>
  </ul>
  ...
  foreach ($groups as $g => $desc) {
    ...
    <div class="tab_content <?php echo (!$first) ? 'hidden' : ''; ?>" id="<?php echo Format::slugify($g); ?>">
      <table class="dashboard-stats table" id="responsive-table">
        <tbody>
          <tr>
            <?php
            $headers = [];
        ...
        <tbody>
          ...
          foreach ($data['data'] as $i => $row) {
            ...
            foreach ($row as $j => $td) {
              ...
              <?php       } else { ?>
                <td data-label="<?php echo Format::htmlchars($headers[$j]); ?>"><?php echo Format::htmlchars($td);
...
<script>
  ...
  let plotData = <?php echo JsonDataEncoder::encode($report->getPlotData()); ?>;

  $.drawPlots(plotData);

  window.addEventListener("resize", function() {
    $("#line-chart-here").empty(); // clear old graph
    $.drawPlots(plotData); // redraw
  });
