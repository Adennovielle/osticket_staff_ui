        ...
        <div class="pull-left">
            <form action="staff.php" method="GET" name="filter">
                ...
                <select name="did" id="did">
                    ...
                    <?php if (($depts = Dept::getDepartments())) {
        ...
        <tbody>
            ...
            ?>
                    <tr id="<?php echo $id; ?>">
                        ...
                        <td data-label="Name"><a href="staff.php?id=<?php echo $id; ?>"><?php echo
                                                                                        Format::htmlchars((string) $agent->getName()); ?></a></td>
                        <td data-label="Username"><?php echo $agent->getUserName(); ?></td>
                        <td data-label="Status"><?php echo $agent->isActive() ? __('Active') : '<b>' . __('Locked') . '</b>'; ?><?php
                                                                                                                                echo $agent->onvacation ? ' <small>(<i>' . __('vacation') . '</i>)</small>' : ''; ?></td>

                        <td data-label="Department"><a href="departments.php?id=<?php echo
                                                                                $agent->getDeptId(); ?>"><?php
                                                                                    echo Format::htmlchars((string) $agent->dept); ?></a></td>
                        <td data-label="Created"><?php echo Format::date($agent->created); ?></td>
                        <td data-label="Last Login"><?php echo Format::relativeTime(Misc::db2gmtime($agent->lastlogin)) ?: '<em class="faded">' . __('never') . '</em>'; ?></td>
                    </tr>
            <?php
                } //end of foreach
            endif; ?>
...
<div style="display:none;" class="dialog" id="confirm-action">
    ...
    <p class="confirm-action" style="display:none;" id="enable-confirm">
        <?php echo sprintf(
            __('Are you sure you want to <b>enable</b> (unlock) %s?'),
            _N('selected agent', 'selected agents', 2)
        ); ?>
    </p>
    <p class="confirm-action" style="display:none;" id="disable-confirm">
        <?php echo sprintf(
            __('Are you sure you want to <b>disable</b> (lock) %s?'),
            _N('selected agent', 'selected agents', 2)
        ); ?>
