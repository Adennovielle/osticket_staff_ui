        ...
        $emitLevel = function ($queues, $level = 0) use ($all_queues, &$emitLevel) {
          ...
          foreach ($queues as $q) { ?>
            <tr>
              ...
              <td data-label="Name" colspan="<?php echo max(1, 5 - $level); ?>"><a
                  href="queues.php?id=<?php echo $q->getId(); ?>"><?php
                                                                  echo Format::htmlchars($q->getFullName()); ?></a>
                <i class="faded-more pull-right icon-sort"></i>
              </td>
              <td data-label="Creator"><?php echo Format::htmlchars($q->staff ? $q->staff->getName() :
                                          __('SYSTEM')); ?></td>
              <td data-label="Status"><?php echo Format::htmlchars($q->getStatus()); ?></td>
              <td data-label="Created"><?php echo Format::date($q->created); ?></td>
            </tr>
        <?php
