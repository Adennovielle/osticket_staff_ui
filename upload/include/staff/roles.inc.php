        ...
        <tbody>
            <?php foreach (
                ...
                <tr>
                    ...
                    <td data-label="Name"><a href="?id=<?php echo $id; ?>"><?php echo
                                                                            $role->getLocal('name'); ?></a></td>
                    <td data-label="Status">&nbsp;<?php echo $role->isEnabled() ? __('Active') :
                                                        '<b>' . __('Disabled') . '</b>'; ?></td>
                    <td data-label="Created On"><?php echo Format::date($role->getCreateDate()); ?></td>
                    <td data-label="Last Updated"><?php echo Format::datetime($role->getUpdateDate()); ?></td>
                </tr>
            <?php }
            ?>
        </tbody>
        <tfoot>
            <tr>
                <td colspan="5">
                    ...
                    <?php } else {
                        echo sprintf(
                            __('No roles defined yet &mdash; %s add one %s!'),
                            '<a href="roles.php?a=add">',
                            '</a>'
                        );
                    } ?>
                </td>
